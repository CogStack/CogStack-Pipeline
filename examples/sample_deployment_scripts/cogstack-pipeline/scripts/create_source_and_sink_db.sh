...
\c $POSTGRES_DATABANK_DB
CREATE TABLE "test_table" (
   
    id BIGINT  NOT NULL PRIMARY KEY ,
    document_timestamp timestamp NOT NULL,
    content text DEFAULT 'none'::text,
...
# import sql.gz file
# gunzip -c $DATA_DIR/$DB_DUMP_FILE | psql -v ON_ERROR_STOP=1 -U $POSTGRES_USER -d $POSTGRES_DATABANK_DB
